...
@router.get("/stream-timing")
async def stream_timing_test():
    """
    Diagnostic: measure wire-level TTFB for English (Vertex) and Assamese (Sarvam)
    using the same clients and settings as production. Runs both in parallel.
    Remove or gate behind auth before going to production.
    """
    import json as _json

    async def _time_vertex_stream() -> dict:
        from app.services.ai.vertex_client import vertex_client, GENAI_BASE_URL, _thinking_config
        import httpx as _httpx
        ...
        gen_cfg = {"maxOutputTokens": 256, **_thinking_config(vertex_client.model)}
        if vertex_client._use_genai_api:
            url = f"{GENAI_BASE_URL}/{vertex_client.model}:streamGenerateContent?key={vertex_client._api_key}&alt=sse"
            payload = {
                "contents": [{"parts": [{"text": "What is photosynthesis? Answer in 2 sentences."}]}],
                "generationConfig": gen_cfg,
            }
            hdrs = {"Content-Type": "application/json"}
        else:
            token = await vertex_client._get_access_token()
            url = f"{vertex_client.base_url}/{vertex_client.model}:streamGenerateContent?alt=sse"
            payload = {
                "contents": [{"role": "user", "parts": [{"text": "What is photosynthesis? Answer in 2 sentences."}]}],
                "generationConfig": gen_cfg,
            }
            ...
            async with _httpx.AsyncClient(timeout=30.0) as c:
                async with c.stream("POST", url, headers=hdrs, json=payload) as resp:
                    if resp.status_code != 200:
                        ...
                        return {"status": "error", "http": resp.status_code, "body": body.decode()[:200]}
                    async for line in resp.aiter_lines():
                        if not line.startswith("data: "): continue
                        raw = line[6:].strip()
                        if raw == "[DONE]" or not raw: continue
                        try: chunk = _json.loads(raw)
                        except: continue
        ...
        return {
            "ttfb_ms": round((first_t - t0) * 1000) if first_t else None,
    ...
    async def _time_sarvam_stream() -> dict:
        from app.config import settings as _s
        import httpx as _httpx

        if not _s.SARVAM_API_KEY:
            return {"status": "skipped", "reason": "SARVAM_API_KEY not set"}
        ...
        payload = {
            "model": _s.SARVAM_MODEL,
            "messages": [
                {"role": "system", "content": "You are a helpful assistant that answers in Assamese."},
                {"role": "user",   "content": "সালোক সংশ্লেষণ কি? ২টা বাক্যত উত্তৰ দিয়া।"},
            ],
            "temperature": 0.7,
            "enable_thinking": False,
            "max_tokens": 256,
        ...
        hdrs = {"api-subscription-key": _s.SARVAM_API_KEY, "Content-Type": "application/json"}
        first_raw_t = None   # first ANY delta (including think)

        try:
            async with _httpx.AsyncClient(timeout=60.0) as c:
                async with c.stream("POST", f"{_s.SARVAM_BASE_URL}/chat/completions", headers=hdrs, json=payload) as resp:
                    if resp.status_code != 200:
                        body = await resp.aread()
                        return {"status": "error", "http": resp.status_code, "body": body.decode()[:300]}
                    async for line in resp.aiter_lines():
                        if not line.startswith("data: "): continue
                        raw = line[6:].strip()
                        if raw == "[DONE]" or not raw: continue
                        ...
                        if content:
                            if first_raw_t is None:
                                first_raw_t = time.monotonic()
                            buffer += content
                            if not in_think and buffer.lstrip().startswith("<think>"):
                                in_think = True
                            if in_think:
                                ...
                                if "</think>" in buffer:
                                    ...
                                    buffer = after
                                    in_think = False
                                    if after.strip() and first_t is None:
                                        first_t = time.monotonic()
        ...
        t_end = time.monotonic()
        had_think = think_chars > 0
        return {
            "ttfb_ms": round((first_t - t0) * 1000) if first_t else None,
            "wire_ttfb_ms": round((first_raw_t - t0) * 1000) if first_raw_t else None,
            "total_ms": round((t_end - t0) * 1000),
            "chars": total_chars,
            "think_chars_stripped": think_chars,
            "think_block_present": had_think,
            "model": _s.SARVAM_MODEL,
            "enable_thinking_sent": False,
        }

    en_result, as_result = await asyncio.gather(
        _safe_check(_time_vertex_stream(), timeout=30.0),
        _safe_check(_time_sarvam_stream(), timeout=60.0),
    )

    return {
        "english_vertex": en_result,
        "assamese_sarvam": as_result,
        "note": "ttfb_ms = first non-think content; wire_ttfb_ms = first raw delta from server",
    }
