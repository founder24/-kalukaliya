...
export default function StaffOperations({ user, subjects = [], chapters = [], selectedSubject, onRefresh, onOpenChapter }) {
  ...
  const [repairQueue, setRepairQueue] = useState(null);
  const [repairQueueError, setRepairQueueError] = useState(null);
  ...
  const loadRepairQueue = useCallback(async () => {
    setRepairQueueError(null);
    try {
      const res = await client().get('/admin/content/ahsec-d1-import/index-repair-queue?limit=25');
      setRepairQueue(res.data && typeof res.data === 'object' ? res.data : { chapters: [] });
    } catch (error) {
      setRepairQueueError(error?.response?.data?.detail || 'Index repair queue is unavailable.');
    }
  }, []);
  useEffect(() => {
    loadImportApprovals();
  ...
  useEffect(() => {
    loadRepairQueue();
    const id = setInterval(loadRepairQueue, 12000);
    return () => clearInterval(id);
  }, [loadRepairQueue]);
  ...
  return <div className="space-y-5 p-4 sm:p-6" data-testid="staff-operations">
    ...
    <section data-testid="index-repair-queue">
      <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-sm font-bold text-slate-900">Index repair queue</h3>
          <p className="mt-1 text-xs text-slate-400">Latest unresolved chapter failures from current and archived import runs</p>
        </div>
        <button type="button" onClick={loadRepairQueue} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50" data-testid="button-refresh-index-repair-queue"><RefreshCw size={14} /> Refresh</button>
      </div>
      {repairQueueError ? (
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">
          <AlertTriangle className="mr-2 inline" size={15} />{repairQueueError}
          <button type="button" onClick={loadRepairQueue} className="ml-2 font-bold underline">Retry</button>
        </div>
      ) : !repairQueue ? (
        <div className="h-24 animate-pulse rounded-2xl bg-slate-100" />
      ) : repairQueue.chapters?.length ? (
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
            <span className="font-semibold text-rose-600">{repairQueue.total} chapter{repairQueue.total === 1 ? '' : 's'} waiting</span>
            <span>·</span>
            <span>next repair attempts are capped at {repairQueue.attempt_limit ?? 3}</span>
            {repairQueue.has_more && <span className="text-amber-700">· showing first {repairQueue.limit}</span>}
          </div>
          {repairQueue.chapters.map(item => (
            <article key={`${item.chapter_id}-${item.failed_at || ''}`} className="rounded-2xl border border-rose-200 bg-rose-50/60 p-4" data-testid={`index-repair-item-${item.chapter_id}`}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-900">
                    <span className="break-all">{item.chapter_id}</span>
                    <span className="rounded-full border border-rose-200 bg-white px-2 py-0.5 text-[10px] font-bold uppercase text-rose-700">{item.operation || 'index'}</span>
                    {item.archived && <span className="rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-bold uppercase text-slate-500">archived run</span>}
                  </div>
                  <p className="mt-1 text-xs text-slate-600">
                    Attempts used: {item.attempts_used ?? 1}/{item.attempt_limit ?? repairQueue.attempt_limit ?? 3}
                    {item.next_attempt ? ` · next attempt ${item.next_attempt}` : ''}
                    {item.failed_at ? ` · failed ${fmt(item.failed_at)}` : ''}
                  </p>
                </div>
                {item.run_id && <span className="max-w-full break-all text-right font-mono text-[10px] text-slate-500">run {item.run_id}</span>}
              </div>
              <div className="mt-3 rounded-xl border border-rose-100 bg-white px-3 py-2">
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">
          No chapters are currently eligible for index repair.
          {repairQueue.exhausted > 0 && <span className="mt-1 block text-xs text-amber-700">{repairQueue.exhausted} chapter{repairQueue.exhausted === 1 ? '' : 's'} reached the repair-attempt limit.</span>}
        </div>
      )}
    </section>
    <section>
