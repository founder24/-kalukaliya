...
staffRouter.get('/content/chapters/:subjectId', async (c) => {
  ...
  const rows = await db.select({
    ...
    pyqPdfUrl: chapters.pyqPdfUrl, pyqPapers: chapters.pyqPapers,
  ...
  return c.json(rows.map(ch => ({
    ...
    // Keep the legacy admin editor's list contract available while callers
    // move to the canonical staff route.
    content:          ch.notesEn ?? '',
    content_as:       ch.notesAs ?? '',
    notes_en:         ch.notesEn ?? '',
    notes_as:         ch.notesAs ?? '',
    notes_generated:  Boolean(ch.notesEn?.trim()),
    slug:             ch.slug,
