...
Validates four things, all over plain DNS lookup (`dig +short`):
  ...
  1. SPF on the parent domain (`syrabit.ai`) authorises SendGrid via
     ...
     delegation pattern).
  2. The SendGrid sending subdomain (default `em.syrabit.ai`) has the
     three CNAMEs SendGrid auto-issues for DKIM signing
     (`s1._domainkey`, `s2._domainkey`, plus a tracking CNAME). The
     specific target hostnames vary per account, so the script only
     checks that the three CNAME targets exist and end in
     `.sendgrid.net.`.
