...
**Task #554 retirement (2026-05-07):** Azure OpenAI (chat / embed / Whisper / text-embedding-3-large) is REMOVED from the dispatch chain. `providers/azure_openai.py`, `gpt-4.1-nano`, the legacy A3 SKU table, the third-tier `Workers-AI Mistral-7B` and `Workers-AI generic gpt-oss-20b` legs are all retired from `PROVIDER_PRIORITY["english_rag_chat"]`. **Task #552 §G-R retirement (2026-05-09):** Azure Speech (TTS / STT) and Azure Translator are now ALSO retired — `providers/azure_speech.py`, `services/backend/azure_ai/{speech,translator}.py`, and the `AZURE_SPEECH_*` / `AZURE_TRANSLATOR_*` env vars are gone. Voice runs ElevenLabs (TTS primary) → Deepgram Aura-2 → Workers-AI; STT runs Deepgram Nova-3 → Workers-AI; translate runs IndicTrans2 → generic Workers-AI. CI guard `scripts/check_dead_providers.py` bans `azure_openai|AzureOpenAI|AZURE_OPENAI_*|gpt-4.1-nano` bare-token across the tree.
