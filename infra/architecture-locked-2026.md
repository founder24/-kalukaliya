...
| Item | Status | Source path(s) | Note |
|---|---|---|---|
| Authentication — Supabase sole IdP (sign-up, sign-in, OAuth, password reset, MFA) | PARTIAL | `artifacts/syrabit-backend/auth_deps.py`, `oidc_auth.py`, `routes/auth.py` | Supabase JWKS verification + OIDC broker live, but legacy email/password endpoints in `routes/auth.py` (`/auth/signup`, `/auth/login`) and the `JWT_SECRET` user-session path in `auth_deps.py` remain active. Cutover owned by a follow-up Supabase migration task; founder lock asserts the destination, this row tracks the in-flight reality. |
