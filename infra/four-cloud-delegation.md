...
| Edge AI dispatch / BYOK gateway | **Cloudflare AI Gateway** | §0, §1 | Slugs: `azure-openai`, `cerebras` (telemetry-parity only, never primary), `cohere` *(retired by #491; row stays for diff history only)*. |
...
- `google_cloud_run_*`, `google_cloud_tasks_*`, `google_cloud_scheduler_*`, `google_compute_*`, `google_container_cluster`, `google_cloudbuild_*`, `google_cloudfunctions_*`, `google_storage_bucket` with `website` block.
- `aws_apprunner_*`, `aws_elastic_beanstalk_*`, or an `aws_ecs_service` whose `name` contains `syrabit-backend` / `syrabit-api` / `fastapi`.
- New Python or TS modules importing `cloud_tasks_client` / `cloud_scheduler_client` (deleted by Task #489).
- New imports / SDK references for **Vertex chat** (`vertex_gemini` outside the content-formatter module added by #494), **Vertex multilingual embedding** (`text-multilingual-embedding-*`), **Vertex Vector Search** (`vertexai.matching_engine`), **Cerebras** outside the CF-AI-Gateway BYOK shim, **Cohere** chat/embed/rerank, or **Voyage AI** anywhere.
...
| Item | Status | Why |
|---|---|---|
| **Deferred-embed reembed Lambda handler** (`sqs_consumers.reembed.handler`) | **Deferred — Python code not yet written.** | The Terraform queue + DLQ + alarms are provisioned in `artifacts/syrabit/infra/aws/sqs-reembed.tf`, but the `aws_lambda_function.reembed_consumer` block + `aws_lambda_event_source_mapping.reembed` are commented out behind a `# TASK-489-FOLLOWUP` marker so we do not deploy a Lambda whose handler does not exist. Producer-side enqueue + alarms are live; the drain side comes in the follow-up task that lands `services/backend/sqs_consumers/reembed.py`. |
| **OTEL → Cloud Trace exporter wiring** (Bicep + Terraform) | **Deferred — runtime exporter env vars not added.** | The GCP IAM grant (`roles/cloudtrace.agent`) and the SA are in place (`artifacts/syrabit/infra/gcp/iam.tf`); the next step is the ACA `infra/azure/aca-syrabit-backend.bicep` env-var addition (`OTEL_TRACES_EXPORTER=googlecloud,sentry` + `OTEL_EXPORTER_GCP_PROJECT_ID=syrabit-prod`) and the matching `infra/aws/lambda-otel.tf` exporter layer. Tracked in the GCP landing-zone runbook §9 (canary workflow `cloud-trace-canary.yml` is also deferred). |
| **Cloud Billing → Meter A/B/C runtime hookup** | **Deferred — meter feed not wired to `gcp_billing.get_*`.** | `gcp_billing.py` already returns the budget summary + per-service spend. The follow-up task removes the `cerebras` / `cohere` / `voyage_ai` / `vertex_embed` rows from `credit_burn_meter.py`'s pool and adds the GCP Billing call to the Meter A/B/C feed loop, then validates the 80 % Slack alert against `#syrabit-oncall`. |
| **S3 → R2 nightly EventBridge sync** | **Deferred — EventBridge schedule + Lambda not provisioned.** | V4 §11 + matrix row "R2 buckets" require S3 to be temp-only with a nightly promotion of finals into R2. The follow-up task adds an `aws_scheduler_schedule` + Lambda that walks the day's `/finals/` prefix, copies to R2, and deletes the S3 source on success. The R2 bucket is already declared in `workers/edge-proxy/wrangler.syrabitworker.toml`; only the AWS-side mover is missing. |
| **SES burn-threshold smoke runbook** | **Deferred — `scripts/ses_burn_smoke.sh` not authored.** | Matrix §C.2 lists the manual `EMAIL_PROVIDER=ses` flip + SQS depth check as the acceptance proof. The follow-up task wraps it as a runnable script under `scripts/`, schedules a quarterly run, and appends the result to `docs/ops/dr-drills/`. |
