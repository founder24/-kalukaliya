...
| **TTS / STT / Indic speech** | **ElevenLabs** (TTS), **Deepgram** (STT primary), **Sarvam** (Assamese chat LLM only) | TTS: Deepgram → Workers AI; STT: AssemblyAI → Workers AI; Indic STT: Google Chirp_2 → Workers-AI Whisper. **All three voice routes (`/tts`, `/stt`, `/voice/voice`) sit behind `Depends(require_paid_plan)`** (Task #549). | Umbrella guard `_check_voice_paywall`; `tests/test_voice_paid_gate.py`. |
| **Indic→English translation fallback** | **Workers-AI IndicTrans2** | **Azure Translator** (REST, paid quota-bounded) → generic Workers AI last-resort. Note: this is the *Translator* resource, distinct from the retired Azure OpenAI tenant. | `tests/test_translate_fallback_chain.py`. |
...
| **Transactional email (today)** | **Azure Marketplace SendGrid** (`EMAIL_PROVIDER=sendgrid`) | **AWS SES** (`EMAIL_FALLBACK=ses`, activated when SendGrid burn-threshold tripped; DKIM/SPF/DMARC published on Cloudflare DNS for both). *(Task #557 collapses this to SES sole tier-1 — see §C.)* | (No umbrella ban yet — see §C TODO.) |
| **Web-push notifications (today)** | **Firebase Cloud Messaging** (`firebase_admin` + `FCM_SERVER_KEY`) | None. *(Task #557 replaces with self-hosted VAPID push — see §C.)* | (No umbrella ban yet — see §C TODO.) |
...
2. **Operator overrides are explicit.** `CHAT_PRIMARY_OVERRIDE`, `RAG_EMBEDDING_PROVIDER_FORCE`, `EMBED_DEGRADED_MODE`, `EMAIL_PROVIDER`, `EMAIL_FALLBACK` — every override is an env-var named in the row above, surfaced in `replit.md`, and ignored when the value is unrecognised (rather than silently degrading).
3. **Azure OpenAI is fully retired** (Task #554). The umbrella bans `azure_openai|AzureOpenAI|AZURE_OPENAI_*|gpt-4.1-nano` bare-token across the tree. The only surviving Azure surfaces are **Azure Speech** (TTS / STT) and **Azure Translator**, on `AZURE_SPEECH_*` / `AZURE_TRANSLATOR_*` keys via `providers/azure_speech.py`.
...
These rows are part of the canonical map but the umbrella guard does **not** ban them yet because the work to migrate the codebase has not landed on `main`. The guard carries explicit `TODO Task #557 / #558` markers so the bans flip on the moment those tasks merge — preventing a documented-but-unenforced row from quietly drifting back in.

| Row | Pending task | What flips on |
|---|---|---|
| Email (SES sole tier-1) | **Task #557** | Bans `sendgrid|SendGridAPIClient|SENDGRID_API_KEY|resend|RESEND_API_KEY` bare-token. SendGrid → SES cutover runbook authored in the task; `EMAIL_PROVIDER=ses` becomes the only supported value. |
| Web-push (self-hosted VAPID) | **Task #557** | Bans `firebase_admin|FCM_SERVER_KEY|FIREBASE_SERVICE_ACCOUNT`. Adds `WEB_PUSH_VAPID_PRIVATE_KEY` / `WEB_PUSH_VAPID_PUBLIC_KEY` to the required-env list. |
