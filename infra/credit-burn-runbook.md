...
| `EMAIL_FALLBACK` env var *(post-#364, used during the §3 warmup window)* | Azure Container App env var on `syrabit-backend`, read by `email_templates.py` `_send_sync` lines 276–298 on every send | infra (set to `ses` for the entire §3 Phase B warmup window so 4xx/421 sendgrid responses route to SES instead of `dropped_perm`; remove after Day-30 checkpoint) | unset (= default 5xx-only escalation policy) | infra | next ACA revision (~60 s) | `az containerapp update --remove-env-vars EMAIL_FALLBACK` |
| SendGrid IP Warmup *(post-#364)* | SendGrid dashboard → Settings → IP Addresses → Warmup toggle (per dedicated IP) | infra (one-time enable; SendGrid throttles outbound itself per the 30-day RFC-aligned schedule) | Off (= no warmup ramp; full volume) | infra | < 30 s (toggle takes effect on the next send; in-flight sends already routed are unaffected) | toggle Warmup → Off in the SendGrid dashboard. **Requires** `EMAIL_FALLBACK=ses` to be live on the backend so 4xx cap-exceeded responses route to SES; without it the warmup drops user mail. |
| SendGrid Sender Reputation Pause *(post-#364)* | SendGrid dashboard → Sender Reputation → Pause Sending | infra (emergency only — used for §3 hard rollback) | Off (= sending) | infra | < 30 s | un-toggle in the SendGrid dashboard. Same `EMAIL_FALLBACK=ses` precondition as the warmup toggle. |
