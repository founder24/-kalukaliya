...
async def _generate_and_clean_quiz(
    *,
    ...
    Provider routing (2026-05-06 user instruction — flipped from the
    legacy Azure-primary order):

    * **English quiz** (``response_lang="en"`` or unset): generated by
      Vertex / Gemini ``gemini-2.5-flash`` as the unconditional
      PRIMARY, with Azure GPT-4.1-mini as the only FALLBACK on Vertex
      outage. The legacy ``prefer_vertex`` kwarg is retained for
      source-level back-compat with ``admin_pipeline._polish_stage_quiz_pregen``
      and ``pregenerate_chapter_quiz`` callers but is now a **no-op** —
      Vertex is the primary for *every* English quiz call regardless
      of how it was scheduled (lazy on-click or polish-stage pre-gen).

    * **Assamese quiz** (``response_lang`` starts with ``"as"``): same
      Vertex-primary English generation step, then a post-generation
      translation pass via Workers-AI IndicTrans2
      (``providers.workers_indic.call_indic_trans``,
      ``@cf/ai4bharat/indictrans2-en-indic-1b``). Each MCQ's question
      text, four choices, and explanation are translated separately
      so a per-field IndicTrans2 failure can fall back to the English
      string for that one field rather than dropping the whole card.
      We do NOT ask the general LLM to write Assamese directly any
      more — purpose-built Indic translators produce materially
      better Assamese (correct script, clean declensions, no English
      residue) than a general chat model attempting it via prompt.

    Returns the cleaned list of question dicts. Raises ``HTTPException``
    on any LLM-side failure so the route handler can propagate the
    same status codes the original inline code did. The Assamese
    translation pass is best-effort — translator failures are logged
    and the English fallback is returned for that field, never a 502."""
    ...
    is_assamese = bool(response_lang and response_lang.lower().startswith("as"))
    # Always generate in English — the Assamese path translates via
    # IndicTrans2 after the cleaned questions are in hand. The legacy
    # "Write in Assamese" instruction was removed because it forced a
    # general LLM to produce low-quality Assamese (transliterated text,
    # English residue) and bypassed the purpose-built translator.
    user_msg_parts = [
    ...
    if is_assamese:
        cleaned = await _translate_quiz_to_assamese(cleaned)
...
async def _translate_quiz_to_assamese(questions: list[dict]) -> list[dict]:
    """Translate cleaned quiz questions into Assamese via Workers-AI
    IndicTrans2 (``@cf/ai4bharat/indictrans2-en-indic-1b``). Each MCQ's
    question text, four choices, and explanation are translated
    separately so a single per-field translator failure (network blip,
    non-Assamese-script regression — see the script-validation guard
    in ``providers.workers_indic.call_indic_trans``) falls back to the
    English string for that one field rather than dropping the whole
    card.

    Best-effort by design — never raises. If the IndicTrans2 provider
    isn't importable (e.g. CF credentials missing in dev), the
    untranslated English questions are returned and a warning is
    logged. Callers may re-translate later via the same helper once
    the translator is healthy."""
    try:
        from providers.workers_indic import call_indic_trans
    except Exception as e:
        logger.warning(
            f"[edu_quiz] IndicTrans2 import failed; returning English: {e}"
        )
        return questions

    async def _tr(s: str) -> str:
        s = (s or "").strip()
        if not s:
            return s
        try:
            return await call_indic_trans(s, direction="en-indic")
        except Exception as e:
            logger.warning(
                f"[edu_quiz] IndicTrans2 per-field failed (len={len(s)}): "
                f"{type(e).__name__}: {e}"
            )
            return s
    ...
    out: list[dict] = []
    for q in questions:
        q_text = await _tr(str(q.get("q", "")))
        choices_out: list[str] = []
        for c in (q.get("choices") or []):
            choices_out.append(await _tr(str(c)))
        expl = await _tr(str(q.get("explanation", "")))
        out.append({
            **q,
            "q": q_text,
            "choices": choices_out,
