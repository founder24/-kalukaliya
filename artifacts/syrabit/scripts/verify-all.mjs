...
// CDN prefix for subject-specific images generated by the og-images pipeline.
// Source of truth: _OG_IMAGE_BASE in workers/edge-proxy/src/index.ts.
const OG_IMAGE_CDN_PREFIX = "https://cdn.syrabit.ai/og/";

// Known site-hosted fallback images (absolute URLs, not a broad prefix, so
// that any unexpected asset path is caught rather than silently allowed).
// Add to this list only when a new canonical og:image is introduced.
const OG_IMAGE_SITE_FALLBACKS = new Set([
  "https://syrabit.ai/opengraph.jpg", // generic SPA-shell fallback
]);

// Combined check helper — returns true if the URL is an allowed og:image value.
const _isAllowedOgImageUrl = (u) =>
  u.startsWith(OG_IMAGE_CDN_PREFIX) || OG_IMAGE_SITE_FALLBACKS.has(u);
...
for (const page of pages) {
  ...
  if (!_isAllowedOgImageUrl(imgUrl)) {
    fail(
      `${route}: og:image URL (${imgUrl}) is not an allowed value. ` +
        `CDN prefix: ${OG_IMAGE_CDN_PREFIX} — Site fallbacks: ${[...OG_IMAGE_SITE_FALLBACKS].join(", ")}. ` +
        `Update OG_IMAGE_SITE_FALLBACKS or OG_IMAGE_CDN_PREFIX in verify-all.mjs if the pipeline has changed.`,
    );
    continue;
  }
