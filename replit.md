...
- **Required env vars (ACA, from Azure KV):** `MONGO_URL`, `JWT_SECRET`, `ADMIN_JWT_SECRET`, `GOOGLE_APPLICATION_CREDENTIALS_JSON`, `SARVAM_API_KEY`, `AZURE_SPEECH_KEY`, `AZURE_TRANSLATOR_KEY`, `RAZORPAY_KEY_SECRET`, `WORKERS_EMBED_SECRET`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `EMAIL_PROVIDER`, `EMAIL_FALLBACK`, `EMBED_PROVIDER_PRIMARY`, `WORKERS_EMBED_URL`. Operator override knobs (optional): `CHAT_PRIMARY_OVERRIDE`, `RAG_EMBEDDING_PROVIDER_FORCE`, `EMBED_DEGRADED_MODE`. *(Task #557 will add `WEB_PUSH_VAPID_PRIVATE_KEY` / `WEB_PUSH_VAPID_PUBLIC_KEY` and remove the SendGrid / FCM keys — see four-cloud-delegation.md §C.)*
