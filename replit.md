...
- **B3 partial state** (2026-05-06): A3 SKU + A9 ordering shipped. Chat-pool top-of-chain *unresolved*: V4 §4 mandates Vertex Gemini 2.5 Flash co-primary for long/high-risk turns (with a token-length + risk-score router) but `config.py:1199-1217` still has Azure as SOLE primary per a 2026-05-05 user instruction. The two same-day signals contradict — see the in-line comment for the rollback trigger if/when the founder re-decides.
