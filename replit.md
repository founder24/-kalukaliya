...
- **PG → Mongo migration** (V4 §13): Phase 1 (ADR) DONE — see `docs/architecture/adr/0001-pg-to-mongo.md` (10-table collection map, per-phase rollback flags, Supabase OAuth blocker). **Phase 2 SHIPPED for `users` collection only** (2026-05-06): `artifacts/syrabit-backend/db_dualwrite.py` helper + `MONGO_USER_WRITES` rollback env flag (default ON) + counters `users.{success,fail,skipped_disabled,skipped_no_db}`; wired into `db_ops` (3 sites) + `atomic_deduct_credit` PG path + 3 route refund mirrors (`ai_chat`, `edu_study`); `routes/admin_monetization.py` 8 paired writes carved out (transactional rollback semantics — see ADR decision log). Other 9 collections still on Phase 1. Phase 3 (read-shadow) → 4 (cutover) → 5 (rip-out) NOT STARTED. Until Phase 4 cutover, V4 is *aspirational* on the user-data SoT axis.
