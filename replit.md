...
- **D1 mirror lag watchdog (Task #460, 2026-05-06):** `routes/admin_d1_mirror_lag_alerts.py` pages on-call when `d1_mirror.lag_snapshot().lag_seconds` (cross-checked against `job_locks[d1_sync_nightly_lease].last_fired_at` so leader hand-offs don't false-positive) exceeds `D1_MIRROR_LAG_THRESHOLD_S` (36h default) for `D1_MIRROR_LAG_REQUIRED_STREAK` consecutive checks (default 2). Loop `_d1_mirror_lag_alert_loop` is leader-gated via `background_lease` (lease id `d1_mirror_lag_alert_lease`), CAS-deduped on `d1_mirror_lag_alert_state`, with 24h re-page debounce + recovery alert + admin pill at `GET /admin/health/d1-mirror/lag`.
- **CF_EDGE_CACHE counters (Task #454, 2026-05-06):** Per-isolate counters in `artifacts/syrabit/workers/edge-proxy/src/index.ts` are now mirrored to a shared KV key `__kv_usage:CF_EDGE_CACHE:<utc-day>:<isolate-id>` (flushed every 10 ops via `ctx.waitUntil`). `/api/edge/kv-usage` lists+sums every isolate's key before responding so `/admin/kv-health` reflects global burn instead of one isolate's view. Falls back to local counters if KV listing fails.
- **Four-cloud delegation lock-in (Task #489, 2026-05-06):** [`infra/four-cloud-delegation.md`](infra/four-cloud-delegation.md) is the canonical "who-owns-what" map across Cloudflare / Azure / AWS / GCP, layered on top of V4 §0–§14 + the §15 amendment from sibling tasks #490/#491/#492. GCP leftover modules deleted (`cloud_tasks_client.py`, `cloud_scheduler_client.py`, `cloudbuild.yaml`, `CLOUDRUN-DEPLOY.md`); call-sites in `routes/admin_gcp_status.py`, `routes/admin_gcp_infra.py`, `gcp_auth.py`, `sqs_fanout.py` updated. Minimal GCP Terraform under `artifacts/syrabit/infra/gcp/` (provider + SA + IAM + budget — no hosting/cron/CI/queueing; Vertex aiplatform API + IAM intentionally NOT enabled here, sibling #494 owns formatter wiring). AWS deferred-embed replay queue (`artifacts/syrabit/infra/aws/sqs-reembed.tf`) is now end-to-end live with Python consumer at `artifacts/syrabit/services/backend/sqs_consumers/reembed.py`. AWS S3→R2 nightly EventBridge sync (`artifacts/syrabit/infra/aws/s3-to-r2-sync.tf` + `services/backend/s3_to_r2_sync.py`) promotes finals at 02:11 UTC. ACA Bicep adds `OTEL_TRACES_EXPORTER=googlecloud,sentry` for Cloud Trace long-retention backstop. SES burn-threshold smoke at `scripts/ses_burn_smoke.sh` (DR drill log → `docs/ops/dr-drills/`). Web Risk wired into `edu_reader.fetch_and_extract` against the post-redirect `final_url` (closes the threat-model publisher-policy rule). CI drift guard `.github/workflows/four-cloud-delegation-drift.yml` + `.github/scripts/four_cloud_delegation_drift.sh` mechanically blocks re-introduction of forbidden patterns (GCS website hosting, GCP IAM Cloud-Run/Tasks/Scheduler/Build/Functions roles, AWS App Runner / EB / ECS-for-API, Vertex chat/embed/Vector-Search, Cohere/Voyage NEW imports, Cerebras chat-primary).
- **Assamese content backfill (Task #465, 2026-05-06):** `aca_jobs/as_translation_backfill.py` is a resumable driver that walks `subjects`, `chapters`, `seo_pages`, `pyq_html_pages` and translates the configured English fields (`name`, `description`, `title`, `topic_title`, `meta_description`, `content`, `content_html` per `FIELD_MAP`) into matching `<field>_as` siblings so `seo_engine._localized()` returns real Assamese instead of the silent English fallback. Translation goes through V4 §4 Assamese chain via `routes.ai_chat._assamese_translate_gemini_main_sarvam_polish` (Workers-AI IndicTrans2 primary → Vertex / Gemini polish; Sarvam intentionally NOT used per the §15 amendment scoping Sarvam to `assamese_rag_chat` only). State per-collection in `as_translation_state` (keyed `_id` = collection name, tracks `last_processed_id`, `running`, `last_run`); idempotent skip when `<field>_as` ≥ `MIN_AS_SCRIPT_RATIO` (0.60) Bengali-letter ratio AND `<field>_as_src_hash` matches the source. CLI driver `scripts/backfill_assamese_content.py` mirrors `backfill_workers_embeddings.py` (`--status`, `--collection`, `--max-docs`, `--pass-size`). Acceptance probe `scripts/probe_assamese_ssr_coverage.py` fetches one Assamese-mode SSR URL per `seo_pages.page_type` plus a chapter + PYQ page and asserts <5% Latin-letter ratio in the article body (exit 2 on any failure). No silent English fallback — translations below the script-ratio gate are dropped, not stored.
- **Vertex scope-down (Task #490, 2026-05-06):** V4 §15 amendment ([`infra/v4-locked-architecture.md`](infra/v4-locked-architecture.md)) locks Vertex into a single surface — the NotebookLM-style content formatter `vertex_format.format_with_vertex(text, *, style, lang)`. Deleted: `vertex_chat.py`, `providers/vertex_embed.py`, `retrievers/vertex.py`, `_call_vertex_chat` / `_stream_vertex_gemini` / SA-OAuth chat helpers in `llm.py`, all `("<pool>", "vertex")` rows from `config.POOL_WEIGHTS`, and Vertex from every `PROVIDER_PRIORITY` list except `content_format`. Added: `content_format=["vertex"]` priority + weight `10000`. Embed failover collapsed to **Option D (cache-only degraded mode)** — the second Pinecone namespace `fallback_vertex_pending_reembed` is gone; primary outage now serves cached vectors and queues fresh content to the AWS SQS deferred-embed consumer (no silent Vertex fallback). Admin retriever-toggle refuses `active="vertex"` (HTTP 400). Contract tests `tests/test_vertex_format_contract.py` + `tests/test_embed_failover_degraded_mode.py` lock the shape; acceptance grep on `_call_vertex_chat|_stream_vertex_gemini|VertexVectorSearchRetriever|VERTEX_INDEX_ID|VERTEX_DEPLOYED_INDEX_ID|fallback_vertex_pending_reembed` is zero outside the §15 changelog. Out of scope: formatter wiring (#494), Cohere/Cerebras removal, Cloud Run leftovers (#489).
