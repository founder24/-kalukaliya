...
deploy_rust_core() {
    ...
    if command -v az &> /dev/null; then
        ...
        bash "$(git rev-parse --show-toplevel)/scripts/azure-deploy.sh" deploy rust-core
    else
        echo -e "${YELLOW}az CLI not found. Manual deployment required.${NC}"
        echo "   1. Install az: brew install azure-cli  (or see docs/DEPLOYMENT.md)"
        echo "   2. Authenticate: az login"
        echo "   3. Deploy: see infra/azure/aca-syrabit-backend.bicep + .github/workflows/azure-container-apps-deploy.yml"
    fi
...
show_next_steps() {
    ...
    echo "  3. Check logs: az containerapp logs show -n rust-core -g <rg> --follow"
    echo "  4. Test JARVIS HUD: https://syrabit.ai/jarvis"
    echo "  5. Test staff login: https://syrabit.ai/staff/login"
    echo ""
    echo "Rollback instructions:"
    echo "  - Azure: az containerapp revision activate -n rust-core -g <rg> --revision <prev>"
