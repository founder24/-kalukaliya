...
1. **OCR has no persisted scratch tier today.** Both OCR endpoints — `/ai/ocr-image` (`artifacts/syrabit-backend/routes/ai_chat.py:566-654`, single-image chat composer) and `/pyq/process` (`artifacts/syrabit-backend/routes/pyq.py`, PYQ pipeline) — read the upload into a bounded in-memory buffer (`_OCR_MAX_BYTES=8MB`), magic-byte sniff, and pass the bytes straight to Vertex Vision (`vertex_services.ocr_image` / `vertex_services.analyze_image`). The extracted text goes into Mongo. No `r2_upload`, no `azure_blob_storage`, no temp-file write.
...
**Action taken.** No code change — there is nothing to migrate. Lock row §4.2 "Azure Blob — temporary OCR/media" is flipped from PARTIAL → **RETIRED** with a note pointing to the in-memory pipeline and to R2 (§4.1) for warm media. The matrix row mirrors the lock. `scripts/check_architecture_lock.py` stays green (RETIRED rows are allowed to have empty `source_paths`). The Azure landing-zone runbook §10 "intentionally not here" gains a row noting OCR scratch is *not* on Azure Blob.
