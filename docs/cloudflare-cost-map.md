...
| ~~Vectorize (`syllabus-index`, 768-dim, legacy)~~ | **Retired (Task #308).** Was a rollback fallback queried from the worker as `SYLLABUS_INDEX_LEGACY`. Removed after a 60-day window with v2 as the sole live index and zero rollback events. The binding was deleted from `wrangler.toml` and the index itself was deleted via `wrangler vectorize delete syllabus-index`; the next monthly cost review should show the corresponding line item drop to zero. | n/a (retired) | n/a |
| AI Gateway (`syrabit-ai-gw`) | **Active** — routes every `env.AI.run(...)` from the edge worker (both fallback fan-out and edge vector-search). Default cache TTL 1h, embeddings overridden to 24h in the dashboard. Provisioned via `workers/edge-proxy/scripts/provision-ai-gateway.mjs`; activated by setting `WORKERS_AI_GATEWAY_ID` as a Wrangler secret on `syrabit-edge`. Watchdog at `workers/edge-proxy/src/ai-gateway-cache-alert.ts` (Task #311) pages on-call via `SYNTHETIC_PROBE_WATCHDOG_WEBHOOK_URL` if the 24h embed cache-hit-rate falls below 50%, so a silent regression doesn't quietly burn the credit pool until the next monthly review. | 🟦 STARTUP-100% (gateway requests are free; cached responses don't re-bill upstream) | **Keep.** Re-run the provisioning script any time the cache policy in the script changes — it is idempotent and overwrites hand-edits in the dashboard, by design. To rotate / disable, see `docs/ops/ai-gateway-activation.md` (rollback section). Monthly cost review (Step 5) confirms the tag list is non-empty and that embeddings show a non-zero cache-hit-rate; the Task #311 watchdog converts that monthly check into a real-time page. |
